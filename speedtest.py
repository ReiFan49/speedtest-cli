...
def parse_args():
    ...
    parser.add_argument('--bytes', dest='units', action='store_const',
                        const=('B', 8), default=('b', 1),
                        help='Display values in bytes instead of bits. Does '
                             'not affect the image generated by --share, nor '
                             'output from --json or --csv')
    parser.add_argument('--show-per', dest='unit_per', action='store_const',
                        const='p', default='/',
                        help='Display speed unit with p instead a /.')
...
def shell():
    ...
    if args.download:
        ...
        printer('Download: %0.2f M%s%ss' %
                ((results.download / 1000.0 / 1000.0) / args.units[1],
                 args.units[0], args.unit_per),
                quiet)
    else:
